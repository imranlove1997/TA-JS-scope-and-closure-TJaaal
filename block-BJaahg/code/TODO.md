...
```js
var one = addOne(0); // ReferenceError: addOne is not defined
const addOne = (num) => {
  return num + 1;
};
var two = addOne(1);
...
```js
console.log(addOne(0)); // ReferenceError: addOne is not defined
const addOne = (num) => {
  return num + 1;
};
var two = addOne(1);
...
sample('First', 'Second', 'Third'); // Hello I am undefined
```
...
outer();
// I love movie call Mad Max: Fury Road
```
...
outer();
// I love movie called BEFORE SUNRISE
```
...
outer();
// I loce this movie called GONE GIRL
```
